...
public class UserLoginWarn {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<MallUserLogin> test = env.addSource(new RMQSource<>(connectionConfig,
                ...
                .setParallelism(1)
                .map(string -> GsonUtil.fromJson(string, MallUserLogin.class)).setParallelism(1);
//                .filter(new FilterFunction<MallUserLogin>() {
//                            @Override
//                            public boolean filter(MallUserLogin mallUserLogin) throws Exception {
////                                Configuration configuration = new Configuration();
////                                configuration.setString("userId",mallUserLogin.getUserId());
                        ...
                        if(logins.size()>0 ){
                            ...
                            for(MallUserLogin login:logins){
                                ...
                                if(!CollectionUtils.isEmpty(list) && !collect.contains(login.getAddress())){
                                    ...
                                    MailUtil.AuthMailSSL("iamcrawler@sina.com",login.getAddress());
